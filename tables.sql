...
-- TRIGGERS AND STORED PROCEDURE
	...
	create Procedure calc_goals(IN ppname varchar(255),mmid INT,tteam varchar(255))

	  BEGIN
	          UPDATE GOALS
	            SET GOALS = GOALS + (SELECT NO_OF_GOALS FROM GOALS_MATCH WHERE PLAYER = ppname and MATCH_ID = mmid and TEAM = tteam)
	            WHERE PLAYER_NAME = ppname;
	  END
	//


	CREATE TRIGGER update_gls
	  AFTER INSERT
	  ON GOALS_MATCH FOR EACH ROW
	    BEGIN
	      IF NEW.PLAYER NOT IN (SELECT PLAYER_NAME FROM GOALS) THEN
	        INSERT INTO GOALS VALUES(NEW.PLAYER,NEW.NO_OF_GOALS);
	      ELSE
	        CALL calc_goals(NEW.PLAYER,NEW.MATCH_ID,NEW.TEAM);
	      END IF;
	    END
	  //
